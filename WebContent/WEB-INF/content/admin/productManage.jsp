...
<div id="wrapper">
  ...
  <form id="searchForm" action="${root }/good/data/showGoodsByFuzzySearchAndPage2.do/${keyWords }/${pageNo }" method="post">
                <div class="input-group">
                    ...
                    <input size="10" id="searchInput" value="${sessionScope.keyWords }" type="text" name="id" class="form-control1 input-search" placeholder="根据商品ID或名称/详情搜索并进行修改">
  ...
  <div class="bs-example">
	              ...
	              <label class="control-label">按照商品种类查找</label>
	            ...
	            <div class="form-group filled">
	              <label class="control-label">按照商品上下架查找</label>
	              	<select id="searchByStat" name="searchByStat" >
...
<form class="form-floating ng-pristine ng-invalid ng-invalid-required ng-valid-email ng-valid-url ng-valid-pattern" novalidate="novalidate" ng-submit="submit()">
          ...
          <table id="jiangfengkx" class="table">
                    ...
                    <tbody>
                    	<c:if test="${sessionScope.goods ne null }">
                    		<c:forEach items="${sessionScope.goods }" var="good" varStatus="vs">
                    	...
                    	<c:if test="${sessionScope.goods eq null }">
...
<script type="text/javascript">
	...
	$( "#category_idFu" ).change( function() {
		...
		sessionStorage.setItem("statOfGoods", null);
		$.ajax( {
			url: "${root}/good/data/getGoodsByCategory_Id2.do/"+categoryId+"/1",
			type: "post",
			dataType: "json",
			success: function( result ) {
		...
		if ( $checks.size() > 0 ) {
			if ( confirm( "确认删除？" ) ) {
				...
				var sendData = {
						...
						"ids": sessionStorage.getItem("checkboxIds")
				};
				$.ajax( {
	...
	$( "#searchButton" ).click( function() {
		...
		sessionStorage.setItem("statOfGoods", null);
		$( "#searchForm" ).attr( "action", "${root }/good/data/showGoodsByFuzzySearchAndPage2.do/"+keyWords+"/1" );
		//alert( $( "#searchForm" ).attr( "action" ) )
	} );
	
	
	var checkboxIds = {  };
	if ( sessionStorage.getItem("checkboxIds") != null ) {
		checkboxIds = JSON.parse(sessionStorage.getItem("checkboxIds"));
	}
	$( "#AllChecks" ).click( function() {
		...
		$.each( $checks, function( index, thiz ) {
			...
			if ( $( thiz ).prop("checked") ) {
				...
				checkboxIds[id] = undefined;
			}
		} );
		//alert(JSON.stringify(checkboxIds))
		sessionStorage.setItem("checkboxIds", JSON.stringify(checkboxIds));
		//alert(sessionStorage.getItem("checkboxIds"))
	...
	if ( sessionStorage.getItem("num") == null ) {
		...
		$( "#numOfSelected" ).text( sessionStorage.getItem("num") );
	}
	...
	// 按照商品上下架查找
	$( "#searchByStat" ).change( function() {
		var stat = this.value;
		if ( stat != -1 ) {
			//alert( stat )
			sessionStorage.setItem("statOfGoods", stat);
			sessionStorage.setItem("category_idFu", null);
			$.ajax( {
				url: "${root}/good/data/searchGoodsByStatAndPage.do/"+stat+"/1",
				type: "post",
				dataType: "json",
				data: { "stat": stat },
				success: function( result ) {
	...
	$( "#searchByStat option[value='"+sessionStorage.getItem("statOfGoods")+"']" ).attr("selected", "selected");
